...
const MisTurnosPaciente = () => {
  ...
  const [vistaActual, setVistaActual] = useState('simples'); // 'simples' | 'periodicos'

  useEffect(() => {
    if (user?.id) {
      fetchTurnos();
      fetchTurnosPeriodicos();
    }
  }, [user]);
  ...
  const handleCancelarPeriodico = async (periodicoId) => {
    if (!window.confirm('¿Cancelar todos los turnos futuros de esta serie?')) return;
    
    try {
      await axios.delete(
        `${API}/turnos-periodicos/${periodicoId}?cancelarSoloFuturos=true`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
...
export default MisTurnosPaciente;
