...
const SeguimientoPublicoPage = () => {
  ...
  const fetchSeguimiento = async () => {
    ...
    try {
      ...
      console.error('Error al cargar seguimiento:', fetchError);
  ...
  const handleSubmitRespuesta = async (e) => {
    ...
    if (preguntasPersonalizadas.length > 0) {
      ...
      const faltanRespuestas = preguntasObligatorias.some(pregunta => {
        const respuestaLocal = respuestasPreguntas[pregunta.id_pregunta];
        if (!respuestaLocal) return true;

        switch (pregunta.tipo_respuesta) {
          case 'texto':
            return !respuestaLocal.texto?.trim();
          case 'escala':
            return respuestaLocal.numerica === null || respuestaLocal.numerica === '';
          case 'sino':
            return respuestaLocal.booleana === null;
          case 'opcion':
            return !respuestaLocal.opcion?.trim();
          default:
            return true;
        }
      });
      ...
      try {
        ...
        preguntasPersonalizadas.forEach(pregunta => {
          ...
          switch (pregunta.tipo_respuesta) {
            case 'texto':
              respuestasFormateadas[pregunta.id_pregunta] = respuestaLocal.texto;
              break;
            case 'escala':
              respuestasFormateadas[pregunta.id_pregunta] = respuestaLocal.numerica?.toString();
              break;
            case 'sino':
              respuestasFormateadas[pregunta.id_pregunta] = respuestaLocal.booleana ? 'Si' : 'No';
              break;
            case 'opcion':
              respuestasFormateadas[pregunta.id_pregunta] = respuestaLocal.opcion;
              break;
            default:
              break;
          }
        ...
        console.error('Error al enviar respuestas:', submitError);
        alert(submitError.response?.data?.message || 'Error al enviar las respuestas');
      } finally {
        setSubmitting(false);
      }
    } else {
      ...
      try {
        ...
        console.error('Error al enviar respuesta:', submitError);
        alert(submitError.response?.data?.message || 'Error al enviar la respuesta');
      } finally {
        setSubmitting(false);
      }
    }
        ...
        return (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              {[...Array(10)].map((_, i) => {
                const valor = i + 1;
                const seleccionado = respuestaLocal.numerica === valor;
                return (
                  <button
                    key={valor}
                    type="button"
            ...
            {respuestaLocal.numerica && (
              <p className="text-sm text-gray-600 text-center">
                Seleccionaste: <span className="font-medium">{respuestaLocal.numerica}</span>
              </p>
            )}
          </div>
        );
          ...
          <div className="flex gap-4">
            <label className="flex items-center cursor-pointer">
              <input
                ...
                checked={respuestaLocal.booleana === true}
            ...
            <label className="flex items-center cursor-pointer">
              <input
                type="radio"
                name={`pregunta_${pregunta.id_pregunta}`}
                checked={respuestaLocal.booleana === false}
            ...
            {pregunta.opciones?.map((opcion, index) => (
              <label key={index} className="flex items-center cursor-pointer">
                <input
                  type="radio"
                  name={`pregunta_${pregunta.id_pregunta}`}
                  checked={respuestaLocal.opcion === opcion}
          ...
          <div className="p-6">
            ...
            {seguimiento?.instrucciones && (
              <div className="mt-6 p-4 bg-blue-50 rounded-lg border border-blue-200">
                <p className="text-sm font-semibold text-blue-900 mb-2">📋 Instrucciones del profesional:</p>
                <p className="text-sm text-blue-800 whitespace-pre-wrap">{seguimiento.instrucciones}</p>
              </div>
            )}
          </div>
        </div>
...
export default SeguimientoPublicoPage;
