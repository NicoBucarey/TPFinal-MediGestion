  ...
  const handleSubmitRespuesta = async (e) => {
    ...
    if (preguntasPersonalizadas.length > 0) {
      ...
      const faltanRespuestas = preguntasObligatorias.some(pregunta => {
        const respuestaPregunta = respuestasPreguntas[pregunta.id_pregunta];
        if (!respuestaPregunta) return true;
        
        switch (pregunta.tipo_respuesta) {
          case 'texto':
            return !respuestaPregunta.texto?.trim();
          case 'escala':
            return respuestaPregunta.numerica === null || respuestaPregunta.numerica === '';
          case 'sino':
            return respuestaPregunta.booleana === null;
          case 'opcion':
            return !respuestaPregunta.opcion?.trim();
          default:
            return true;
        }
      });
      ...
      try {
        ...
        const respuestasFormateadas = preguntasPersonalizadas.map(pregunta => {
          ...
          return {
            ...
            respuesta_texto: pregunta.tipo_respuesta === 'texto' ? respuestaPregunta.texto : null,
            respuesta_numerica: pregunta.tipo_respuesta === 'escala' ? parseInt(respuestaPregunta.numerica) : null,
            respuesta_booleana: pregunta.tipo_respuesta === 'sino' ? respuestaPregunta.booleana : null,
            respuesta_opcion: pregunta.tipo_respuesta === 'opcion' ? respuestaPregunta.opcion : null
          };
        });
        ...
        return (
          <div className="space-y-3">
            ...
            <div className="flex gap-2">
              {[...Array(10)].map((_, i) => {
                const valor = i + 1;
                const seleccionado = respuestaPregunta.numerica === valor;
                return (
                  <button
                    key={valor}
                    type="button"
            ...
            {respuestaPregunta.numerica && (
              <p className="text-sm text-gray-600 text-center">
                Seleccionaste: <span className="font-medium">{respuestaPregunta.numerica}</span>
              </p>
            )}
          </div>
        );
          ...
          <div className="flex gap-4">
            <label className="flex items-center cursor-pointer">
              <input
                ...
                checked={respuestaPregunta.booleana === true}
            ...
            <label className="flex items-center cursor-pointer">
              <input
                type="radio"
                name={`pregunta_${pregunta.id_pregunta}`}
                checked={respuestaPregunta.booleana === false}
            ...
            {pregunta.opciones?.map((opcion, index) => (
              <label key={index} className="flex items-center cursor-pointer">
                <input
                  type="radio"
                  name={`pregunta_${pregunta.id_pregunta}`}
                  checked={respuestaPregunta.opcion === opcion}
...
export default MisSeguimientosPage;
